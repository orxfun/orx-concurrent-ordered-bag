...
#[cfg(not(miri))]
const NUM_BATCHES: [usize; 5] = [1, 2, 4, 1024, 2587];
#[cfg(miri)]
const NUM_BATCHES: [usize; 4] = [1, 2, 4, 124];

#[test_matrix([1, 4, 64, 128])]
fn write_at_odd_even_batches(batch_size: usize) {
...
#[test_matrix([1, 4, 64, 128])]
fn non_uniform_batches(batch_size: usize) {
    for num_batches in NUM_BATCHES {
        let bag = ConcurrentOrderedBag::new();
        let con_bag = &bag;
...
#[test_matrix([1, 2, 4, 8])]
fn early_alloc_batch(num_additional_threads: usize) {
    for num_batches in NUM_BATCHES {
        let bag = ConcurrentOrderedBag::new();
        let con_bag = &bag;
